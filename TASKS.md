...
- 2026-03-09: E4-T2 を完了。`app/(protected)/articles/page.tsx` と `utils/articles.ts` を追加し、記事一覧・ページ別 KPI・直近 14 日推移・流入クエリを 1 画面で見られる記事分析画面を実装。`page_daily` / `query_daily` は latest batch のみ採用するよう `sql/bigquery/page_daily.sql` と `sql/bigquery/query_daily.sql` を修正して重複集計を解消し、`components/studio-nav.tsx` で nav の active 判定も追加。`npx next typegen` と `npm run build` 成功を確認
