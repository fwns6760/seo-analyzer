...
- [x] E4-T3 クエリ分析画面
- [x] E4-T4 改善候補一覧画面
- [x] E4-T5 loading / error / not-found 実装
- [x] E4-T6 共通レイアウト実装
...
- [x] E6-T4 batch/job 側の build / deploy 自動化方針整理
- [x] E6-T5 Secret / 環境変数 / サービスアカウントの CI 用整理
- [x] E6-T6 GitHub push から本番反映までの動作確認
...
- 2026-03-09: E6-T6 を完了。`~/.ssh/sebata1413` を `ssh-agent` に読み込んだ状態で `git push origin main` を実行し、commit `4f74939` を push。`GitHub Actions` から `Artifact Registry` に image tag `4f74939724dc59925a18a4d35def53e774608348` が push され、`Cloud Run` `seo-analyzer-web` は revision `seo-analyzer-web-00002-sn9` へ更新された
- 2026-03-09: E6-T4 を完了。`.github/workflows/deploy-job.yml` を追加し、`main` push または manual run を起点に Docker build -> `Artifact Registry` push -> `Cloud Run Jobs update` を行う workflow を実装。`seo-batch-deployer` Service Account と OIDC provider `seo-analyzer-batch` を追加し、push 後に `seo-fetch-job` が image tag `9347980a45ca39d8aa5f6af5554d9d99504fd603`、generation `6` へ更新された
- 2026-03-09: Web / batch workflow に path filter を追加し、docs-only push や片側変更で不要な再デプロイが走らないように整理
- 2026-03-09: E4-T1 を完了。`app/page.tsx` を認証済みダッシュボードへ置き換え、`BigQuery` の `page_daily` / `improvement_candidates_base` を server 側から読んで KPI・改善候補・上位ページを表示する構成を実装。`utils/google-auth.ts` と `utils/bigquery.ts` で `metadata server -> gcloud` fallback の認証を追加し、`seo-web-runtime` には `roles/bigquery.jobUser` と `roles/bigquery.dataViewer` を付与。`sql/bigquery/improvement_candidates_base.sql` の previous-only 二重化バグも修正し、`npm run build` 成功まで確認
- 2026-03-09: E4-T6 を完了。認証済みルートを `app/(protected)` へ移し、`app/(protected)/layout.tsx` で Looker Studio 風の sidebar / topbar / canvas を共通化。ダッシュボードは新 shell 前提で再配置し、`npm run build` 成功と push 後の `Cloud Run` 最新 revision 反映まで確認
- 2026-03-09: E4-T2 を完了。`app/(protected)/articles/page.tsx` と `utils/articles.ts` を追加し、記事一覧・ページ別 KPI・直近 14 日推移・流入クエリを 1 画面で見られる記事分析画面を実装。`page_daily` / `query_daily` は latest batch のみ採用するよう `sql/bigquery/page_daily.sql` と `sql/bigquery/query_daily.sql` を修正して重複集計を解消し、`components/studio-nav.tsx` で nav の active 判定も追加。`proxy.ts` から現在 path を保護レイアウトへ渡すようにして `/articles` からのログイン復帰も維持。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E4-T3 を完了。`app/(protected)/queries/page.tsx` と `utils/queries.ts` を追加し、クエリ一覧・クエリ別 KPI・直近 14 日推移・紐づくページ一覧を 1 画面で見られるクエリ分析画面を実装。sidebar nav に `/queries` を追加し、記事分析の流入クエリ table から query detail へ遷移できるよう整理。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E1-T8 の本番不具合を修正。`Cloud Run` では `request.url` が内部 origin `0.0.0.0:8080` になるため、Google OAuth 開始時の `redirect_to` が誤っていた。`utils/request-url.ts` を追加して `x-forwarded-host` / `x-forwarded-proto` から公開 URL を組み立て、`/auth/login` `/auth/callback` `/auth/signout` の redirect を修正
- 2026-03-09: MVP 3画面の UIUX を分析ツール寄りに改善。共通 shell に route ごとの context strip を追加し、`dashboard/articles/queries` に jump links、highlight cards、順位バッジ、delta tone、スキャンしやすいテーブル表現を追加。`npm run build` 成功を確認
- 2026-03-09: E4-T4 を完了。`app/(protected)/opportunities/page.tsx` と `utils/opportunities.ts` を追加し、`improvement_candidates_base` を元に `伸びた記事 / 順位下落 / リライト候補 / カニバリ候補` を切り替えられる改善候補一覧画面を実装。sidebar nav に `/opportunities` を追加し、ダッシュボードの候補カードから一覧画面へ遷移できるよう整理。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E4-T5 を完了。`app/loading.tsx`、`app/error.tsx`、`app/global-error.tsx`、`app/not-found.tsx` と `app/(protected)/loading.tsx`、`app/(protected)/error.tsx`、`app/(protected)/not-found.tsx` を追加し、`articles / queries / opportunities` では無効な search param や存在しない対象で `notFound()` へ落とすよう整理。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E5-T1 を完了。`utils/opportunity-rank-drop.ts` に順位下落の共通判定条件を追加し、前週表示 50 以上・前週 20 位以内・順位悪化 1.0 以上に加えて、クリックまたは sessions が 20% 以上かつ 3 以上落ちた page のみを候補化するよう更新。`utils/dashboard.ts` と `utils/opportunities.ts` で同じ条件を共有し、`docs/data_source_contract.md` にルールを明記。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E5-T2 を完了。`utils/opportunity-growth.ts` に伸びた記事の共通判定条件を追加し、今週表示 50 以上・現在 20 位以内・クリックまたは sessions が 20% 以上かつ 3 以上伸び、さらに順位改善または表示増がある page のみを候補化するよう更新。`utils/dashboard.ts` と `utils/opportunities.ts` で同じ条件を共有し、`docs/data_source_contract.md` にルールを明記。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E5-T3 を完了。`utils/opportunity-rewrite.ts` にリライト候補の共通判定条件を追加し、今週表示 80 以上・平均順位 6-20 位・CTR 12% 未満の page を候補化するよう更新。`utils/dashboard.ts` と `utils/opportunities.ts` で同じ条件を共有し、`docs/data_source_contract.md` にルールを明記。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E5-T4 を完了。`utils/opportunity-cannibal.ts` にカニバリ候補の共通判定条件を追加し、今週 2 ページ以上に分散し、表示 80 以上かつ平均順位 20 位以内の query を候補化するよう更新。`utils/opportunities.ts` と改善候補一覧の説明文で同じ条件を共有し、`docs/data_source_contract.md` にルールを明記。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: E5-T5 を完了。`docs/data_source_contract.md` に `mvp_opportunity_threshold_policy` を追加し、`rank_drop / growth / rewrite / cannibal` のしきい値の役割分担と重なり方を整理。MVP の既定閾値セットとして扱う方針を明記した
- 2026-03-09: 実データ確認を実施。`BigQuery` で `improvement_candidates_base` を直接確認した結果、`reference_end_date` は `2026-03-05`、`page` の前週比較列はまだ `0` 行で、現在の候補件数は `rewrite 1件 / growth 0件 / rank-drop 0件 / cannibal 0件` だった。`utils/opportunities.ts` に `comparisonReady` を追加し、`app/(protected)/opportunities/page.tsx` と `app/(protected)/page.tsx` で比較期間不足時は「候補なし」ではなく蓄積中メッセージを出すよう更新。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: 比較待ち UI を調整。`utils/comparison-window.ts` を追加し、`page_daily` の `active_days` と `latest_date` から前週比較 ready の見込み日を計算するようにした。`app/(protected)/page.tsx` と `app/(protected)/opportunities/page.tsx` では `蓄積中 x/14日 -> 最短 yyyy-mm-dd` を表示し、比較不足時の空状態にも同じ見込み日を出すよう更新。`npx next typegen` と `npm run build` 成功を確認
- 2026-03-09: `npm run data:readiness` を追加。`scripts/data-readiness-check.mjs` で `raw_gsc/raw_ga4/page_daily/improvement_candidates_base` の最新日付と比較 ready 状態を 1 回で確認できるようにした。実行結果は `reference_end_date = 2026-03-05`、`page_daily_active_days = 3/14`、`candidate_page_previous_rows = 0` で、前週比較 ready の最短見込みは `2026-03-16`。`node --check` と `npm run build`、実データでの `npm run data:readiness -- --json` 成功を確認
- 2026-03-09: `Epic 2` の status を `Done` に更新。`E2-T1` から `E2-T7` まで全 task 完了済みのため、残件は実装ではなく `data:readiness` による運用確認のみと整理した
