...
### Notes
- Blocker: ローカル環境の `git push origin main` が `git@github.com: Permission denied (publickey)` で失敗中。`origin` への GitHub 認証を解消してから `E6-T6` を再開する
- Owner: me
...
- 2026-03-09: E6-T6 は着手したが、`git push origin main` が `git@github.com: Permission denied (publickey)` で失敗。workflow 実行前に GitHub への push 認証を解消する必要あり
